...
const ProductCard = ({ id, name, imageUrl, subtitle, price, altText, priority = false }) => {
  return (
    <Link href={`/product/${id}`} className="block h-full w-full">
      <div className="group relative w-full max-w-full overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm hover:shadow-xl transition-shadow duration-300 flex flex-col justify-between h-full">
        <div className="relative aspect-square w-full overflow-hidden bg-gray-100">
          ...
          <div className="absolute inset-0 bg-black/50 opacity-0 transition-opacity duration-300 group-hover:opacity-100 flex items-center justify-center p-2">
            <span className="bg-white text-[#B88E2F] font-semibold text-xs sm:text-sm px-3 py-2 sm:px-6 sm:py-3 rounded shadow hover:bg-gray-100 transition-colors text-center max-w-full truncate">
              View product
            </span>
          </div>
        </div>
